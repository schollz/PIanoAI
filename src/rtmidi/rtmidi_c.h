...
*/
RTMIDIAPI int rtmidi_get_compiled_api(
    enum RtMidiApi *apis); // return length for NULL argument.
 ...
 */
RTMIDIAPI void rtmidi_open_port(RtMidiPtr device, unsigned int portNumber,
                                const char *portName);
 ...
 */
RTMIDIAPI const char *rtmidi_get_port_name(RtMidiPtr device,
...
RTMIDIAPI void rtmidi_in_set_callback(RtMidiInPtr device,
                                      RtMidiCCallback callback, void *userData);
...
//! during input.
RTMIDIAPI void rtmidi_in_ignore_types(RtMidiInPtr device, bool midiSysex,
...
RTMIDIAPI int rtmidi_out_send_message(RtMidiOutPtr device,
